#!/usr/bin/env python3
"""Move only Franka to the pinned V60 palm-catch reset at reduced speed.

The reviewed segmented Franka-only reset core is reused. RH56, camera,
perception, and policy modules are never opened. Hardware motion additionally
requires ``--execute`` and five exact operator confirmations.
"""
...
SWEEP_CLEAR_TOKEN = "FR3_RH56_CURRENT_TO_V60_PALMCATCH_RESET_SWEEP_CLEAR"
STOP_TOKEN = "FR3_STOP_READY"
PLA_TOKEN = "PLA_LOW_SPEED_UNLOADED_ONLY"
TARGET_TOKEN = "V60_PALMCATCH_Q_HOME_SHA_VERIFIED"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--confirm-v60-target", metavar=TARGET_TOKEN)
    return parser
...
def _require_confirmations(args: argparse.Namespace) -> None:
    required = (
        ...
        ("--confirm-v60-target", args.confirm_v60_target, TARGET_TOKEN),
    )
    missing = [
        f"{flag} {token}"
...
def _target(task_config: Optional[Path]) -> tuple[np.ndarray, str, str, Path]:
    if task_config is None:
        resolved, _metadata = materialize_task_config("thrown_object_v60")
    else:
    ...
    if (
        ...
        or "V60PalmCatchTilt30" not in task.teacher
    ):
        raise ValueError("task config is not the pinned V60 alpha=0.5 candidate")
    target = np.asarray(task.franka_q_home_rad, dtype=np.float64)
    if target.shape != (7,) or not np.all(np.isfinite(target)):
        raise ValueError("V60 q_home is malformed")
...
def _reset_config(config: dict, target: np.ndarray) -> dict:
    ...
    if np.any(target <= lower) or np.any(target >= upper):
        raise ValueError("V60 q_home violates the pinned FR3 reset margin")
    return result
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        q6_margin = float(target[5] - FR3_SYSTEM_LIMITS[5, 0])
        print(
            f"[config] {config_path}\n"
            f"[adapter] verified={assets.mesh_path} sha256={assets.mesh_sha256}"
        )
        print(
            "[V60 reset] q_home={} q_home_sha256_f64_le={} "
        ...
        print(
            ...
            "segments; RH56/policy/perception unopened; J6 system-limit "
            f"clearance={q6_margin:.6f}rad"
        )
        if not args.execute:
        ...
        print(
            "[path authority] current-to-V60 reset sweep and physical stop "
            "readiness confirmed",
            flush=True,
        )
        ...
        if not np.array_equal(np.asarray(proof.target_q_rad), target):
            raise RuntimeError("Franka reset proof target differs from V60 contract")
        print(
            "[PASS] Franka reached V60 palm-catch q_home; "
